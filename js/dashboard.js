...
/* Currency symbol — SAR in English, ﷼ in Arabic */
function currencySymbol() { return i18n.current === 'ar' ? '﷼' : 'SAR '; }
...
function renderKPIs(data, kpis) {
  ...
  set('kpiRevenue',   kpis.avgMonthlyRevenue > 0 ? currencySymbol() + formatNum(Math.round(kpis.avgMonthlyRevenue)) : 'N/A');
...
function buildCreatorContext(data, kpis) {
  ...
  return [
    ...
    `Avg monthly revenue: ${currencySymbol()}${Math.round(kpis.avgMonthlyRevenue)}`,
    ...
    options: {
        ...
        tooltip: {
          ...
          callbacks: {
            label: ctx => {
              ...
              if (unit === '﷼') return ` Revenue: ${currencySymbol()}${formatNum(v)}`;
      ...
      scales: {
        ...
        y: {
          ...
          ticks: { callback: v => unit === '﷼' ? currencySymbol() + formatNum(v) : v + unit, font: { size: 11 } },
        },
      },
    }
  });
  ...
  el.innerHTML = `
    ...
    <div class="plan-projections-summary">
      ...
      ${plan.revenueProjection ? `<div class="proj-item">
        ...
        <span class="proj-value">${currencySymbol()}${formatNum(plan.revenueProjection.monthly)}<span style="font-size:.8rem;font-weight:500;opacity:.7">/mo</span></span>
        <span class="proj-sub">${i18n.t('plan.range')}: ${currencySymbol()}${formatNum(plan.revenueProjection.min)} – ${currencySymbol()}${formatNum(plan.revenueProjection.max)}</span>
      </div>` : ''}
    </div>
...
function renderSimulationResult(r) {
  ...
  const revHTML = r.simRevenue ? `
    <div class="sim-compare-item">
      ...
      <div class="sim-before">${currencySymbol()}${formatNum(baseline.revenueProjection.monthly)}</div>
      <div class="sim-arrow">→</div>
      <div class="sim-after" style="color:${r.simRevenue.delta >= 0 ? 'var(--accent-green)' : 'var(--accent-red)'};">
        ${currencySymbol()}${formatNum(r.simRevenue.monthly)}
        <span class="sim-delta">(${r.simRevenue.delta >= 0 ? '+' : ''}${currencySymbol()}${formatNum(r.simRevenue.delta)})</span>
      </div>
    </div>` : '';
      ...
      <div class="pricing-main-card">
        ...
        <div class="price-range">
          <div class="price-tier">
            ...
            <div class="price-tier-value">${currencySymbol()}${formatNum(p.minPrice)}</div>
          </div>
          <div class="price-tier ideal">
            <div class="price-tier-label">${i18n.t('pricing.ideal.label')}</div>
            <div class="price-tier-value">${currencySymbol()}${formatNum(p.idealPrice)}</div>
          </div>
          <div class="price-tier">
            <div class="price-tier-label">${i18n.t('pricing.premium.label')}</div>
            <div class="price-tier-value">${currencySymbol()}${formatNum(p.premiumPrice)}</div>
          </div>
        </div>
        ...
        <div class="pricing-breakdown">
          ...
          <div class="pricing-breakdown-row">
            <span>${i18n.t('pricing.base.price')} (${formatNum(p.followers)} followers × ${currencySymbol()}${p.cpmRate} CPM ÷ 1K)</span>
            <span>${currencySymbol()}${formatNum(p.base)}</span>
          </div>
          <div class="pricing-breakdown-row">
            <span>${i18n.t('pricing.eng.mult')} (${p.engMultiplier}×) — ${p.tier}</span>
            <span>${p.engMultiplier >= 1 ? '+' : ''}${currencySymbol()}${formatNum(Math.round(Math.abs(p.engMultiplier - 1) * p.base))}</span>
          </div>
          <div class="pricing-breakdown-row">
            <span>${i18n.t('pricing.growth.factor.row')} (${p.growthFactor}×) — ${gfLabel}</span>
            <span>${p.growthFactor >= 1 ? '+' : '-'}${currencySymbol()}${formatNum(Math.round(Math.abs(p.growthFactor - 1) * p.base * p.engMultiplier))}</span>
          </div>
          <div class="pricing-breakdown-row">
            <span>${i18n.t('pricing.stability.factor')} (${p.stabilityFactor}×) — ${sfLabel}</span>
            <span>${p.stabilityFactor >= 1 ? '+' : '-'}${currencySymbol()}${formatNum(Math.round(Math.abs(p.stabilityFactor - 1) * p.base * p.engMultiplier * p.growthFactor))}</span>
          </div>
          <div class="pricing-breakdown-row total">
            <span>${i18n.t('pricing.adjusted.base')}</span>
            <span>${currencySymbol()}${formatNum(p.adjusted)}</span>
          </div>
      ...
      <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <div class="card">
          ...
          <div style="font-size:1.4rem;font-weight:800;margin:8px 0;">
            ${p.avgRevPerPost !== null ? currencySymbol() + Math.round(p.avgRevPerPost) : i18n.t('pricing.no.data')}
          </div>
...
• Sponsored post: ${currencySymbol()}${pricing.minPrice}–${currencySymbol()}${pricing.idealPrice} per post
• Content series (3 posts): ${currencySymbol()}${Math.round(pricing.idealPrice * 2.6)}
• Long-term retainer (monthly): ${currencySymbol()}${Math.round(pricing.idealPrice * 3.5)}
    ...
    report.innerHTML = `
      ...
      <div class="brand-report-grid">
        ...
        <div class="brand-report-card">
          ...
          <div class="brand-metric-row"><span class="brand-metric-label">Min Budget / Post</span><span class="brand-metric-value">${currencySymbol()}${formatNum(pricing.minPrice)}</span></div>
          <div class="brand-metric-row"><span class="brand-metric-label">Ideal Budget / Post</span><span class="brand-metric-value" style="color:var(--accent-purple);">${currencySymbol()}${formatNum(pricing.idealPrice)}</span></div>
          <div class="brand-metric-row"><span class="brand-metric-label">Premium Budget / Post</span><span class="brand-metric-value">${currencySymbol()}${formatNum(pricing.premiumPrice)}</span></div>
...
function _localChat(text) {
  ...
  if (/revenue|إيراد|عائد|money|ربح/.test(q)) {
    ...
    let msg = isAr
      ? `متوسط الإيرادات الشهرية: ${currencySymbol()}${formatNum(Math.round(kpis.avgMonthlyRevenue))}.\nتقلب الإيرادات: ${vol}% (${stab}).`
      : `Avg monthly revenue: ${currencySymbol()}${formatNum(Math.round(kpis.avgMonthlyRevenue))}.\nRevenue volatility: ${vol}% (${stab}).`;
