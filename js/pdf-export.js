...
window.downloadReport = async function () {
  ...
  try {
    /* ── Ensure charts are rendered; disable animation so canvas is fully painted ── */
    const hasCharts = window.charts && Object.values(window.charts).some(c => c);
    if (typeof getFilteredData === 'function' && typeof renderCharts === 'function') {
      ...
      if (data.length > 0) {
        ...
        if (sec)  { sec.classList.remove('hidden'); sec.style.position = 'fixed'; sec.style.left = '-9999px'; }
        ...
        if (sec) { sec.style.position = ''; sec.style.left = ''; }
